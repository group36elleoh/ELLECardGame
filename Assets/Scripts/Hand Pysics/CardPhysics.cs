...
public class CardPhysics : MonoBehaviour
{
    ...
    // private bool artAndSoundLoaded = false;
    ...
    void Start()
    {
        ...
        if (!GameManager.pictureMode)
        {
            ...
            if (File.Exists(path))
            {
                byte[] _bytes = System.IO.File.ReadAllBytes(path);
                float[] data = new float[_bytes.Length / 4];
                // Buffer.BlockCopy(_bytes, 0, data, 0, data.Length);
                //data = _bytes.Select(b => (float)Convert.ToDouble(b)).ToArray();
                data = ConvertByteToFloat(_bytes);
                cardSound = AudioClip.Create("", _bytes.Length, 1, 44100, false);
                cardSound.SetData(data, 0);
            }
            else
            {
    ...
    */
    ...
    void FixedUpdate()
    {
        ...
        if (!GameManager.pictureMode)
        {
            if (!GameManager.soundDictionary.TryGetValue(GetComponent<AnswerObject>().answerID, out cardSound))
            {
                cardSound = null;
                // Debug.Log("Trying to load sound with ID " + GetComponent<AnswerObject>().answerID + " and failed!");
            }
            else
            {
                // Debug.Log("Loading ID: " + GetComponent<AnswerObject>().answerID + " myClip: " + "\"" + cardSound.name + "\"");
            }
        }
    ...
    void OnMouseEnter()
    {
        ...
        if (cardSound != null && canPlaySound)
            soundPlayer.AddVoice(cardSound);
        // AudioSource.PlayClipAtPoint(tempSound, Camera.main.transform.position, PlayerPrefsManager.GetVoiceVolume());
    }
