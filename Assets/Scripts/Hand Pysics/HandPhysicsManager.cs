...
public class HandPhysicsManager : MonoBehaviour
{
    ...
    void Update()
    {
        //Debug.Log("Current Correct Pairs: " + DeckManager.currentCorrectPairs());
        if (drawBool)
        {
            drawBool = false;
    ...
    void Draw(answer drawnAnswer)
    {
        ...
        spawnedCard = Instantiate(testCardPF, new Vector2(-15, 1), Quaternion.identity);

        spawnedCard.GetComponent<CardPhysics>().currentHandNode = smallestOpenNode;
        spawnedCard.transform.SetParent(GameObject.Find("CardObjects").transform);
    ...
    public static void moveNodes(int nodeCount)
    {
        ...
        foreach (GameObject handNode in handNodeGOs)
        {
            if (nodeCounter < nodeCount)
            {
                ...
                handNode.transform.position = new Vector2(Mathf.Floor(-firstNodeX * nodeSpacing) + nodeCounter * (nodeSpacing), 1);
