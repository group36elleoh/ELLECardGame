...
public class DeckManager : MonoBehaviour
{
    ...
    {
        //Debug.Log("Drawing Smart Answer");
        // Gets list of all current active questions' answers
        List<int> currentCorrectAnswerIDs = new List<int>();

        Debug.Log("I AM LOOKING THROUGH THE CUR QUESTIONS FOR ANSWERS");
        Debug.Log("Current Questiosns: ");
        foreach (question q in QnAManager.currentQuestions)
        {
            Debug.Log(q.getQuestionID() + " " + q.getQuestionPhrase());
        }
        foreach (question q in QnAManager.currentQuestions)
        {
            //Debug.Log("Question IDs: " + q.getQuestionID());
            foreach (int ID in q.getAnswerIDList())
            {
                if(ID != 0)
                {
                    Debug.Log("Current question ID: " + q.getQuestionID() + "\n" +
                        "Adding correct Answer: " + ID);
                    currentCorrectAnswerIDs.Add(ID);
                }
                
            }
        }

        Debug.Log("Current answer list: ");
        foreach (int e in currentCorrectAnswerIDs)
        {
            Debug.Log("Current Answer IDs: " + e);
        }
        ...
        //Debug.Log(" Loops through the list of current correct answers, finds one that is present in the answer deck, and draws the player that answer");
        foreach (int i in currentCorrectAnswerIDs)
        {
            if (answerDeck.Contains(CollectionManager.answerCollection[i]))
            {
                //Debug.Log(CollectionManager.answerCollection[i].getAnswerID());
